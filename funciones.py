...
def load_cat(path):
    ...
    PVtype = pd.read_excel(path,sheet_name='PVModules',header=0,index_col=0)
    # Cargar datos de módulos de Batt
    Battype = pd.read_excel(path,sheet_name='BattModules',header=0,index_col=0)
    
    # Cargar datos de Convertidores híbridos
    ConH = pd.read_excel(path,sheet_name='Hybrid OnGrid',header=0,index_col=0)
    
    WindGens = pd.read_excel(path,sheet_name='WindTurbines',header=0,index_col=0)

    return PVtype, Battype, ConH, WindGens
...
def get_data_fromNSRDB(lat, lon, year):
    ...
    attributes = 'ghi,wind_speed,air_temperature'
    # Choose year of data
    #year = '2019'
...
def calculate_WT_power(df, WindGens, z0, height, elevation):
    ...
    for i in Turbine_profile.columns:
        Profiles[i] = pd.read_excel('Catalogo/Catalogo.xlsx',sheet_name = i,header=0,index_col=0)
